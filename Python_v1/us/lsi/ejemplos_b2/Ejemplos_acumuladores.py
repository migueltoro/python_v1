...
def numero_de_lineas(file: str) -> int: #stdev(ls) hace lo mismo
    return len(lineas_de_fichero(file,encoding='utf-16'))

def numero_de_palabras_distintas(file: str,encoding:str) -> int: #len(distinct(re.split(sep,line))) hace lo mismo
    sep = r'[ ,;.\n():?!\"]'
...
def lista_de_fichero(file:str,encoding:str,delim:str)->list[int]: # similar a lista_de_csv pero devuelve una lista de enteros
...
def test5():
    ...
    print(f"La desviacion tipica de la progresion aritmetica de {a} a {b} con razon {c} es {stdev(range(a,b,c))}")
